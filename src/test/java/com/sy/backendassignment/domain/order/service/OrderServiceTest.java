...
import static com.sy.backendassignment.domain.order.PaymentMethod.CREDIT_CARD;
...
class OrderServiceTest {
    ...
    class CalculatePaymentAmount {
        ...
        void VIP_등급은_1000원_할인된다() {
            ...
            given(orderItem.getPrice()).willReturn(BigDecimal.valueOf(10000));

            // when
            BigDecimal result = orderService.calculatePaymentAmount(vipMember, orderItem, CREDIT_CARD);
        ...
        void VVIP_등급은_10퍼센트_할인된다() {
            // given
            OrderItem orderItem = mock(OrderItem.class);
            given(orderItem.getPrice()).willReturn(BigDecimal.valueOf(50000));

            // when
            BigDecimal result = orderService.calculatePaymentAmount(vvipMember, orderItem, CREDIT_CARD);

            // then
            assertThat(result).isEqualByComparingTo(BigDecimal.valueOf(45000));
        }
        ...
        void 일반_등급은_할인되지_않는다() {
            ...
            // when
            BigDecimal result = orderService.calculatePaymentAmount(normalMember, orderItem, POINT);
        ...
        void 결제금액이_마이너스가_되지_않아야_한다() {
            // given
            OrderItem orderItem = mock(OrderItem.class);
            given(orderItem.getPrice()).willReturn(BigDecimal.valueOf(500));
